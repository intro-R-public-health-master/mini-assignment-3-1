...
**Due Sunday, October 13 @ 11:59pm through Github**
...
For each file, first try the default options in `read_csv()` to read in the file.  Then make any modifications you might need to address the information I've given you about the file. You should view each imported data set after you're done to be sure it looks right!
...
This is the raw clicker data from our chocolate experiment that was eventually processed to provide the data for the Module 3 project.  Click on the file and select "View File" to see what's in the file. All variables are text values but missing values in this file are designated with `NV`.
```{r}
# with default settings
...
This file gives the locations of all of the red light cameras in Baltimore City (from Open Baltimore [https://data.baltimorecity.gov/Transportation/Red-Light-Cameras/ydjd-febd](website)).  Click on the file and select "View File" to see what's in the file.  Be sure to address the installation date variable to be sure it's designated a date.
```{r}
# with default settings
